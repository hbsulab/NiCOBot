...
from NiCOBot.utils import is_smiles
from .utils import cdk, get_image_path_from_folder
...
class LLMThoughtChem(LLMThought):
    ...
    def on_tool_end(
        ...
        **kwargs: Any,
        ...
        if serialized["name"] == "Query2ReactionSimilarity":
            folder_path = 'similarity_reactions_E_Nu_Connected'
            image_path = get_image_path_from_folder(folder_path) 
            st.image(image_path, caption='The left side shows reactions searched based on similarity, and the right side shows the reactions found in the references.')
    def on_tool_start(
